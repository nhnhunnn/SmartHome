...
client.on('message', (topic, message) => {
    if (topic === 'esp32/datasensor') {
        ...
        const { temperature, humidity, lumninosity } = data;
        if (!temperature || !humidity || !lumninosity || 
            temperature < 0 || temperature > 100 || 
            humidity < 0 || humidity > 100 || 
            lumninosity < 0 || lumninosity > 4095) {
            return;
        }

        const sqlInsert = 'INSERT INTO sensir (temp, hum, light, time) VALUES (?, ?, ?, NOW())';
        const valuesInsert = [temperature, humidity, lumninosity];
