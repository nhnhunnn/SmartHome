...
router.post('/action', async (req, res) => {
  ...
  try {
    ...
    // Set a flag to check if response has been sent
    let responseSent = false;

    // Listen for MQTT response
    const receivedMessage = await new Promise((resolve) => {
      mqttClient.on('message', (receivedTopic, message) => {
        if ((receivedTopic === 'esp32/ledStatus' || receivedTopic === 'esp32/fanStatus') && !responseSent) {
          const device_id = receivedTopic === 'esp32/ledStatus' ? 'led' : 'fan';
          const data = message.toString();
          const status = data === "1" ? 1 : 0;
    ...
    // Check if response has not been sent (timeout case)
    if (!responseSent) {
      res.status(500).json({ error: 'Internal Server Error' });
    }
  } catch (error) {
