...
function displayTable(page, data, type) {
    ...
    updatePaginationAction(page, totalItems);
}

function updatePaginationAction(currPage, totalItems) {
    ...
    const createPageLinkAcion = (page, text, isDisabled = false) => {
    ...
    if (currPage >= 1) {
        createPageLinkAcion(1, "<<", currPage === 1);
        createPageLinkAcion(currPage - 1, "<", currPage === 1);
    }
    for (let i = startPage; i <= endPage; i++) {
        createPageLinkAcion(i, i);
    }
    if (currPage <= pageCount) {
        createPageLinkAcion(currPage + 1, ">", currPage === pageCount);
        createPageLinkAcion(pageCount, ">>", currPage === pageCount);
    }
};
